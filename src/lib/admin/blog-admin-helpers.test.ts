...
describe('admin taxonomy document ids', () => {
  it('creates deterministic per-value ids so shared defaults are not overwritten', () => {
    ...
    assert.match(adminAuthorDocumentId('فريق نجم سبا'), /^blog-author-[0-9a-f]{16}$/);
    ...
    assert.notEqual(adminAuthorDocumentId('د. أحمد'), adminAuthorDocumentId('فريق نجم سبا'));
  });
});
...
describe('readAdminPostPayload', () => {
  it('preserves Lexical contentJson for editor round-trips', () => {
    ...
    assert.equal(payload.contentHtml, '');
    assert.equal(payload.featured, true);
    assert.equal(payload.coverWidth, 1200);
    assert.equal(payload.coverHeight, undefined);
  });

  it('rejects missing Lexical contentJson and ignores client HTML', () => {
    assert.throws(() => readAdminPostPayload({
      title: 'عنوان',
      slug: 'slug',
      excerpt: 'excerpt',
      contentHtml: '<p>html</p>',
      contentJson: '   ',
    }), /محتوى المقال غير صالح/);
